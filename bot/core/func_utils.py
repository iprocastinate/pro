...
async def getfeed(link, index=0):
    try:
        ...
        category = (entry.get('nyaa_category') or 
        ...
        feed_data = {
            ...
            'size': size,
            'seeders': int(entry.get('nyaa_seeders', 0) or entry.get('seeders', 0) or 0),
            'leechers': int(entry.get('nyaa_leechers', 0) or entry.get('leechers', 0) or 0),
            'info_hash': entry.get('nyaa_infohash') or entry.get('infohash', ''),
            'category': category,
        }
        
        LOGS.debug(f"Feed entry keys: {list(entry.keys())}")
        LOGS.debug(f"Extracted data: Size={feed_data['size']}, Category={feed_data['category']}")
...
async def mediainfo(file, get_json=False, get_duration=False):
    try:
        ...
        process = await create_subprocess_shell(f"mediainfo {quote(file)} --Output={outformat}", stdout=PIPE, stderr=PIPE)
        stdout, stderr = await process.communicate()
        
        if get_duration:
            try:
                data = jloads(stdout.decode())
                duration = float(data['media']['track'][0]['Duration'])
                return duration if duration > 0 else 1440
            except Exception as e:
                LOGS.debug(f"mediainfo duration parsing failed: {str(e)}, using fallback")
                return 1440
        return await get_telegraph(stdout.decode())
    except Exception as err:
        LOGS.debug(f"mediainfo error: {str(err)}")
        if get_duration:
            return 1440 
        return ""
